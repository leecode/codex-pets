Create a single horizontal sprite strip for the Codex app digital pet `bt-buddy` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Compact chibi BT-inspired titan robot companion: not too fat, olive and off-white armored panels, orange accent plates, cyan horizontal face light, cyan circular chest core, broad shoulders, long chunky forearms, narrow waist, heavy segmented legs and boot feet, right-side antenna, no overhead clamp or hook, no readable text, no faction logos..
